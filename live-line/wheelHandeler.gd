...
func open():
	...
	tween.parallel().tween_property(menu, "global_position", Vector2(1152-168.0,0),1).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property($levelUpScreen/Node2D, "global_position",Vector2(976,-304.0),0.5).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CIRC)
